...
const ConfirmAppointment = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      setAppointments(
        pretifyAppointmentsData(res.data.document[0].createdAppointments)
      );
    } catch (err) {
      console.log(err);
    }
      ...
      <View style={styles.container}>
        <View style={styles.topSection}>
          ...
          <Text style={styles.description}>
            You can choose the date and time from the available doctor's
            schedule
          </Text>
        </View>
        ...
        <ChooseDay
          activeKey={key}
          setActiveKey={setKey}
          data={appointments ? Object.values(appointments) : []}
        />
